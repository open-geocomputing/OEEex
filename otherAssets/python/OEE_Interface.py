...
class oeelChain:
	...
	def __call__(self, *args, **kwargs):
		...
		if not kwargs:
			return ee_Js2Py(oeeJS.oeelCall(ee_Py2Js(self.attr_chain), ee_Py2Js([ee_Py2Js(value) for value in args])))
		else:
			return ee_Js2Py(oeeJS.oeelCall(ee_Py2Js(self.attr_chain), ee_Py2Js([{ee_Py2Js(value) for key, value in kwargs}])))
	
	def requireJS(self, path):
		sourceCode=oeeJS.oeeRequireJS( ee_Py2Js(path))
...
class eeExportModule:
	...
	def __call__(self, *args, **kwargs):
		# Here you could call the function you want with the collected attributes and parameters
		if not kwargs:
			val=oeeJS.oeelExport(ee_Py2Js(self.attr_chain), ee_Py2Js([ee_Py2Js(value) for value in args]))
		else:
			val=oeeJS.oeelExport(ee_Py2Js(self.attr_chain), ee_Py2Js([{ee_Py2Js(value) for key, value in kwargs}]))
		class eeTask:
			def status(self):
...
class eeDataModule:
	...
	def __getattr__(self, attr):
		class eeDataFucntion:
			...
			def __call__(self, *args, **kwargs):
				if not kwargs:
					return ee_Js2Py(oeeJS.oeelData(ee_Py2Js(self.dataFunction), ee_Py2Js([ee_Py2Js(value) for value in args])))
				else:
					return ee_Js2Py(oeeJS.oeelData(ee_Py2Js(self.dataFunction), ee_Py2Js([{ee_Py2Js(value) for key, value in kwargs}])))
			def __str__(self):
...
class EEimpport():
	...
	def getEEPath(self,path,obj=None,currentPath=""):
		...
		if(splitPath[0] in obj):
			...
			result=ee_Js2Py(oeeJS.importPakageStruct(currentPath.lstrip("/")+"/"+splitPath[0]))
			obj[splitPath[0]]={}
	...
	def find_module(self, fullname, path=None):
		...
		if fullname.startswith("users") and not (os.path.exists(fullNameAsPath) or os.path.exists(fullNameAsPath+'.py')):
			...
			if(isinstance(downloadPath,str)):
				...
				code=oeeJS.importCode(downloadPath)
...
def eePrint(*toPrints):
	...
	for ix in range(len(toPrints)):
		if(isinstance(toPrints[ix], ee.computedobject.ComputedObject)):
			...
			toPrints[ix]=ee_Py2Js(toPrints[ix]);
	oeeJS.oeePrint(ee_Py2Js(toPrints))

def eeMapOp(name,args):
	oeeJS.oeeMap(name, ee_Py2Js([ee_Py2Js(value) for value in args]))
if withPlotly:
	def displayPlt(bbox_inches="tight", dpi=100):
		...
		oeeJS.oeePlot("data:image/png;base64,"+encoded_image);
...
def isEEObject(val):
	return oeeJS.oeeIsEE(val);

def EEtype(val):
	return oeeJS.oeeEEtype(val);

def isJSFunction(val):
	return oeeJS.oeeIsFunction(val);

def ee_Js2Py(val):
	if isEEObject(val):
		return getattr(ee,EEtype(val))(ee.deserializer.decode(oeeJS.oeeEncodeEE(val).to_py()));
	if isJSFunction(val):
		def wrapper(*args,**kwargs):
			if not kwargs:
				return oeeJS.callJSFucntion(val,ee_Py2Js([ee_Py2Js(value) for value in args]));
			else:
				return oeeJS.callJSFucntion(val,ee_Py2Js([{key:ee_Py2Js(value) for key, value in kwargs.items()}]));
		return wrapper;
	return val.to_py();
...
def ee_Py2Js(val):
	if isinstance(val, ee.computedobject.ComputedObject):
		#EE object
		return oeeJS.oeeAsEEJS(ee.serializer.encode(val),val.name());
	...
	return oeeJS.oeeAsJS(val)

def getJsCallable(fun):
	idVal=id(fun)
	mappingFunction[idVal]=fun;
	return oeeJS.oeelGetJsCallable(idVal)
