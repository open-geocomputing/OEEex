...
let  taskPanel=null;
let  GEEUserAssetRoot=null;
let maxParallelGSUpload=10;
let parallelGSUpload=0;
let toGSuploadList=[];

let maxParallelDownload=10;
let parallelDownload=0;
let toDownloadList=[];

let listImAvailable={}
let listImAvailableTime={}
...
function uploadTreeFolder(item,path){
	...
	let fileArray={};
	let readEntries=function(entries) {
		let manifest=null;
...
function addManifestToIngestInGEE(manifest,uploadEvents){
	...
	for (let i = uploadEvents.length - 1; i >= 0; i--) {
...
function updateDispaly(uploadDic,chunkSize){
	...
	for (let i = uploadDic.uploadEvents.length - 1; i >= 0; i--) {
...
export function initializeMT(){
	...
	document.addEventListener("UwM_settings",function(event){
		if(event.detail?.parallelUpload)
			maxParallelGSUpload=event.detail.parallelUpload;
		if(event.detail?.parallelUpload)
			maxParallelDownload=event.detail.parallelDownload;
	})
}

function applyConfig(data){
	if (Object.keys(data).length === 0) return;
	document.dispatchEvent(new CustomEvent("UwM_settings",{detail:data}))
}

function getConfig(){

	let configList=['parallelUpload','parallelDownload']
