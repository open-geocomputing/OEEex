...
let editor=null;
let aceRange=null;
function handleSessionAction(sessionId, password, sessionType) {
	...
	if("host"==sessionType ){
		...
		peer.on('open', function(id) {
			...
			document.dispatchEvent(new CustomEvent("startCodeShareSession",{detail:{action:"started"}}))
	...
	if("join"==sessionType ){
		...
		peer.on('open', function(id) {
			...
			con.on('data', function(data){
				...
				if(data.type && data.type=="wrongPassword"){
					document.dispatchEvent(new CustomEvent("startCodeShareSession",{detail:{action:"wrongPassword"}}))
				}
		...
		peer.on('error', function(err) {
			document.dispatchEvent(new CustomEvent("startCodeShareSession",{detail:{action:"wrong"}}))
			peer.destroy()
		});
	}
}
...
export function initializeMT(extensionId){
	document.addEventListener("startCodeShareSession",function(e){
		let param=e.detail;
		if(param.action=="open")
			handleSessionAction( param.sessionIdVal, param.assword, param.sessionType);
	});
}
...
function createSharedCodeSessionPopup(){
	...
	validateButton.addEventListener('click', function() {
		...
		const sessionType = sessionTypeSelector.value; // 'host' or 'join'
		sessionId.classList.remove("wrong");
		passwordSession.classList.remove("wrong");
		document.dispatchEvent(new CustomEvent("startCodeShareSession",{detail:{
			action:"open",
		...
		document.addEventListener("startCodeShareSession",function(e){
			let param=e.detail;
			if(param.action=="wrongPassword")
				popup.querySelector("#sessionPassword").classList.add("wrong");
			if(param.action=="started")
				popup.classList.remove("visible");
			if(param.action=="wrong")
				popup.querySelector("#sessionId").classList.add("wrong")
		});
...
function injectPeer(OEEexidString){
...
function injectDiff(OEEexidString){
	var s = document.createElement('script');
...
export function initialize(){
	injectPeer(chrome.runtime.id);
	injectDiff(chrome.runtime.id);
